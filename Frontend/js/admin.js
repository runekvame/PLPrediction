...
async function scoreSeasonPredictions() {
    ...
    if (teams.length !== 20) {
        showAlert(`Du må skrive inn nøyaktig 20 lag (du har ${teams.length})`, 'error');
        return;
    }
...
async function loadUsers() {
    ...
    const supabaseKey = 'eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6ImZ2emNjdWhwY2ttb2F1cmh4Znp5Iiwicm9sZSI6ImFub24iLCJpYXQiOjE3NzkwNTQ0MDgsImV4cCI6MjA5NDYzMDQwOH0.lUV1tXVGoh8vRN0QziUPycqN_rSet-HNRr-YkeNVPKQ';

    const res = await fetch(`${supabaseUrl}/rest/v1/users?select=username,total_points,created_at&order=total_points.desc`, {
        headers: {
            'apikey': supabaseKey,
